...
export class StyleComponent extends Base implements OnInit{
    ...
    getElements(): Array<[Element, Element, Element]>{
        ...
        for (let i = 0; i < this.style.elements.length; i += 3){
            const first = this.style.elements[i];
            const second = this.style.elements[i + 1] == undefined ? null : this.style.elements[i + 1];
            const third = this.style.elements[i + 2] == undefined ? null : this.style.elements[i + 2];
